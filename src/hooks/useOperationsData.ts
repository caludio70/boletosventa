...
import { fetchOperationsFromDb, saveOperationsToDb, hasOperationsInDb } from '@/lib/supabaseOperations';
import { setOperations, getAllOperations } from '@/lib/realData';
import { toast } from 'sonner';

// Default data to seed the database if empty
const defaultOperations: Partial<OperationRow>[] = [
...
export function useOperationsData() {
  ...
  // Import data and save to database
  const importData = useCallback(async (data: Partial<OperationRow>[]) => {
    setIsSaving(true);
    try {
      const success = await saveOperationsToDb(data);
      if (success) {
        setOperations(data);
        setRefreshKey(prev => prev + 1);
        toast.success(`${data.length} operaciones importadas y guardadas`);
        return true;
      } else {
